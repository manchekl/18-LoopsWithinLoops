...
def draw_L(window, circle, r, c):
    ...
    for i in range(r+3):
        ...
        x = orig_x  # Reset x to the left-edge, for the next row
...
def draw_wall_on_right(rectangle, n, window):
    ...
    for c in range(n+1):
        for r in range(c):
            new_rec = rg.Rectangle(right,left)
            new_rec.attach_to(window)
            window.render(0.1)
            right.x=right.x-rectangle.get_width()
            left.x=left.x-rectangle.get_width()
        right.y=right.y+rectangle.get_height()
        left.y=left.y+rectangle.get_height()
        right.x=orig_ptR
        left.x=orig_ptL
